...
export default function ListaMascotas() {
    ...
    async function confirmarEliminarUsuario() {
        try {
            ...
            for (const m of pacientes.filter(p => p.emailUsuario === usuarioElim.correo)) {
    ...
    return (
        <div>
            <div className="d-flex j-cont-bet align-item f-wrap gap-1 mb-2">
                ...
                <button className="btn-primary" onClick={() => navigate("/nuevo-expediente")}>
                    + Nuevo registro
                </button>
            </div>
                        ...
                        <tbody>
                            {filtrados.map((u, i) => (
                                <tr key={u.correo}>
                                    ...
                                    <td>{u.telefono || "-"}</td>
                                    <td>
                                        <span className={`badge ${(conteos[u.correo] ?? 0) > 0 ? "info" : "secondary"}`}>
                                            {conteos[u.correo] ?? 0} mascota{(conteos[u.correo] ?? 0) !== 1 ? "s" : ""}
                                        </span>
                                    </td>
                                    <td>
                                        <div className="d-flex gap-1 f-wrap">
                                            ...
                                            <button
                                                ...
                                                onClick={() => navigate("/usuarios/editar", {
                                                    state: { usuario: u },
                                                })}
                                            >
                                                Editar
                ...
                <div className="modal-alert is-open">
                    <div className="modal-content">
                        <div className="modal-header">
                            ...
                            <button className="modal-close" onClick={() => setUsuarioElim(null)}>X</button>
                        </div>
                        <div className="modal-body">
                            <p>
                                ¿Eliminar a <strong>{usuarioElim.nombre_completo}</strong> y todas sus mascotas?
                                Esta acción no se puede deshacer.
                            </p>
                        </div>
                        <div className="modal-footer">
                            <button className="btn-outline-secondary" onClick={() => setUsuarioElim(null)}>Cancelar</button>
                            <button className="btn-alert" onClick={confirmarEliminarUsuario}>Sí, eliminar</button>
                        </div>
                    </div>
                </div>
            )}
