...
const TablaServicios = () => {
  ...
  const handleGuardar = async (e) => {
    ...
    if (parseFloat(formulario.precio) <= 0) {
      alert(
        "Error: Las prestaciones médicas de la clínica deben tener un precio mayor a $0.00.",
      );
      return;
    }
    ...
    const datosServicio = {
      ...
      disponible:
        formulario.disponible === true || formulario.disponible === "true",
    };
    ...
    <div>
      ...
      <div className="card card-light shadow-sm br-1 mb-2">
        <div className="card-header fw-bold text-primary">
          {formulario.id
            ? `Modificar Servicio Clínico (ID: ${formulario.id})`
            : "Dar de Alta Nueva Prestación"}
        </div>
        <form
          onSubmit={handleGuardar}
          className="card-body d-flex f-wrap gap-1 align-item"
        >
          <div className="form-group f-2">
            <input
              type="text"
              className="input"
              placeholder="Nombre de la Cirugía o Tratamiento"
          ...
          <div className="form-group f-1">
            <select
              ...
              onChange={(e) =>
                setFormulario({ ...formulario, disponible: e.target.value })
              }
            >
      ...
      {cargando ? (
        ...
        <div className="alerta primary text-center">
          El catálogo está vacío. Comienza a registrar servicios desde cero.
        </div>
      ) : (
                ...
                <tr key={servicio.id}>
                  ...
                  <td>
                    <div className="d-flex gap-1">
                      <button
                        onClick={() => handleSeleccionarEditar(servicio)}
                        className="btn btn-secondary btn-xs"
                      >
                        Editar
                      </button>
                      <button
                        onClick={() => handleBorrar(servicio.id)}
